...
The testing strategy combines **unit/property-based tests** for business logic and **integration tests** for infrastructure wiring.

### Property-Based Testing with jqwik

PBT applies to this feature because it contains pure functions (priority matrix, SLA deadline computation, JWT claim extraction), universal invariants (status transitions, event envelopes, soft-delete filtering), and round-trip operations (token issuance/validation, bcrypt hashing).

**Library**: [jqwik](https://jqwik.net/) — a property-based testing library for Java/JUnit 5.

**Configuration**: Each `@Property` test runs a minimum of **100 tries** (configured via `@Property(tries = 100)`).

**Tag format**: Each property test is annotated with:
```java
@Tag("Feature: glpi-microservices-backend, Property N: <property_text>")
```

**Example — Property 19 (Priority Matrix)**:
```java
@Property(tries = 100)
@Tag("Feature: glpi-microservices-backend, Property 19: priority matrix computation")
void priorityMatrixComputationIsCorrect(
    @ForAll @IntRange(min = 1, max = 5) int urgency,
    @ForAll @IntRange(min = 1, max = 5) int impact
) {
    int[][] matrix = PriorityMatrixService.DEFAULT_MATRIX;
    int computed = priorityMatrixService.compute(urgency, impact, DEFAULT_ENTITY_ID);
    assertThat(computed).isEqualTo(matrix[urgency - 1][impact - 1]);
}
```

**Example — Property 26 (SLA Deadline)**:
```java
@Property(tries = 100)
@Tag("Feature: glpi-microservices-backend, Property 26: SLA deadline excludes non-business hours")
void slaDeadlineNeverFallsInNonBusinessHours(
    @ForAll("validStartInstants") Instant start,
    @ForAll @LongRange(min = 3600, max = 86400) long durationSeconds
) {
    Instant deadline = deadlineService.computeDeadline(start, durationSeconds, DEFAULT_CALENDAR_ID);
    assertThat(calendarService.isBusinessHour(deadline, DEFAULT_CALENDAR_ID)).isTrue();
}
```

**Example — Property 12 (Permission Bitfield Round-Trip)**:
```java
@Property(tries = 100)
@Tag("Feature: glpi-microservices-backend, Property 12: permission bitfield round-trip")
void permissionBitfieldRoundTrip(
    @ForAll @IntRange(min = 0, max = 63) int bits
) {
    Profile profile = profileRepository.save(Profile.withRights("ticket", bits));
    Profile loaded = profileRepository.findById(profile.getId()).orElseThrow();
    assertThat(loaded.getRights().get("ticket")).isEqualTo(bits);
}
```
...
Unit tests cover:
- Specific examples for each use case (happy path + error path)
- Edge cases: empty collections, null fields, boundary values
- Domain service logic with mocked ports
- Status transition matrix validation

**Target**: Each use case class has a corresponding `*UseCaseTest` with at minimum one happy-path and one error-path test.

### Integration Tests

Integration tests (using `@SpringBootTest` + Testcontainers) cover:
- MongoDB repository operations (CRUD, index usage)
- Kafka producer/consumer wiring (embedded Kafka)
- REST endpoint contract tests (MockMvc or WebTestClient)
- SLA escalation scheduler execution
- Notification retry mechanism

**Testcontainers**: MongoDB and Kafka containers are started per test class using `@Container` annotations.

### Test Module Structure

```
src/test/java/com/glpi/{service}/
├── domain/
│   └── service/        # Unit tests for domain services
├── application/
│   └── usecase/        # Unit tests for use cases (mocked ports)
├── adapter/
│   ├── in/rest/        # REST controller integration tests
│   └── out/
│       ├── persistence/ # Repository integration tests (Testcontainers)
│       └── messaging/   # Kafka producer/consumer tests
...
| Use cases | ≥ 85% line coverage |
| REST adapters | ≥ 80% line coverage |
