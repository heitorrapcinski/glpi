...
- [-] 2. Fix all microservice Dockerfiles to copy all module POMs
  ...
  - [~] 2.5 Update `change-service/Dockerfile` to copy all 10 module pom.xml files before running Maven
  - [~] 2.6 Update `asset-service/Dockerfile` to copy all 10 module pom.xml files before running Maven
  - [~] 2.7 Update `sla-service/Dockerfile` to copy all 10 module pom.xml files before running Maven
  - [~] 2.8 Update `notification-service/Dockerfile` to copy all 10 module pom.xml files before running Maven
  - [~] 2.9 Update `knowledge-service/Dockerfile` to copy all 10 module pom.xml files before running Maven
  - [~] 2.10 Verify no changes to docker-compose.yml, pom.xml files, or source code
  - [~] 2.11 Commit: `2026.000003.2: copy all module poms in dockerfiles for maven reactor validation`
