...
/**
 * Kafka consumer for asset domain events.
...
public class AssetEventConsumer {
    ...
    public AssetEventConsumer(NotificationDispatchUseCase dispatchUseCase) {
        this.dispatchUseCase = dispatchUseCase;
    }

    @KafkaListener(topics = "assets.events", groupId = "notification-service")
    public void consume(Map<String, Object> event) {
        String eventType = (String) event.get("eventType");
        log.info("Received asset event: {}", eventType);

        String mappedEventType = mapEventType(eventType);
        if (mappedEventType == null) {
            log.debug("Ignoring unmapped asset event type: {}", eventType);
            return;
        }
    ...
    private String mapEventType(String eventType) {
        ...
        return switch (eventType) {
            case "LicenseOverused" -> "asset.license.overused";
            default -> null;
        };
    }
}
