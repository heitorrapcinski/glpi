...
/**
 * Kafka consumer for problem domain events.
 * Requirements: 16.1, 16.2
 */
@Component
public class ProblemEventConsumer {

    private static final Logger log = LoggerFactory.getLogger(ProblemEventConsumer.class);

    private final NotificationDispatchUseCase dispatchUseCase;

    public ProblemEventConsumer(NotificationDispatchUseCase dispatchUseCase) {
        this.dispatchUseCase = dispatchUseCase;
    }

    @KafkaListener(topics = "problems.events", groupId = "notification-service")
    public void consume(Map<String, Object> event) {
        String eventType = (String) event.get("eventType");
        log.info("Received problem event: {}", eventType);

        String mappedEventType = mapEventType(eventType);
        if (mappedEventType == null) {
            log.debug("Ignoring unmapped problem event type: {}", eventType);
            return;
        }
    ...
    private String mapEventType(String eventType) {
        ...
        return switch (eventType) {
            case "ProblemCreated" -> "problem.created";
